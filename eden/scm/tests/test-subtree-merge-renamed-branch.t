...
test source directory is renamed
  ...
  $ sl subtree copy --from-path foo --to-path bar -m "subtree copy foo -> bar"
  copying foo to bar
  $ echo "x1_foo\nx2\nx3\nx4\nx5" > foo/x
  $ sl ci -m "x1_foo"
  $ echo "x1\nx2\nx3_bar\nx4\nx5" > bar/x
  $ sl ci -m "x3_bar"
  $ sl subtree copy --from-path foo --to-path foo1 -m "subtree copy foo -> foo1"
  copying foo to foo1
  $ echo "x1_foo\nx2\nx3\nx4\nx5_foo1" > foo1/x
  $ sl ci -m "x5_foo1"
  $ showgraph 
...
merge the changes of foo and foo1 to bar
  $ sl log -r 'subtreemergebase("foo1", "bar")' -T '{node|short}\n'
  9032e003042c
  $ sl subtree merge --from-path foo1 --to-path bar
...
merge the change of bar to foo1
  $ sl log -r 'subtreemergebase("bar", "foo1")' -T '{node|short}\n'
  9032e003042c
  $ sl subtree merge --from-path bar --to-path foo1
  searching for merge base ...
  found the last subtree copy commit 52cd91b6716a
  merge base: 9032e003042c
...
test both source and dest directories are renamed
  ...
  $ echo "x1_foo\nx2\nx3\nx4\nx5_foo1\nx6\nx7" > foo1/x
  $ sl ci -m "x5_foo1"
  $ sl subtree copy --from-path bar --to-path bar1 -m "subtree copy bar -> bar1"
  copying bar to bar1
  $ echo "x1\nx2\nx3_bar\nx4\nx5\nx6\nx7_bar1" > bar1/x
  $ sl ci -m "x4_bar1"
  $ showgraph 
...
merge the changes of foo and foo1 to bar1
  $ sl log -r 'subtreemergebase("foo1", "bar1")' -T '{node|short}\n'
  22a57b56f0ee
  $ sl subtree merge --from-path foo1 --to-path bar1
  searching for merge base ...
...
merge the change of bar1 and bar to foo1
  $ sl log -r 'subtreemergebase("bar1", "foo1")' -T '{node|short}\n'
  22a57b56f0ee
  $ sl subtree merge --from-path bar1 --to-path foo1
  searching for merge base ...
  found the last subtree copy commit e6969568e795
  merge base: 22a57b56f0ee
