...
HGPLAIN disables color
  $ HGPLAIN=1 sl status --color=debug
  ? a/1/in_a_1
  ? a/in_a
  ? b/1/in_b_1
  ? b/2/in_b_2
...
HGPLAINEXCEPT=color does not disable color
  $ HGPLAINEXCEPT=color sl status --color=debug
  [? a/1/in_a_1|status.unknown]
  [? a/in_a|status.unknown]
  [? b/1/in_b_1|status.unknown]
  [? b/2/in_b_2|status.unknown]
...
sl status with template
  $ sl status -T "{label('red', path)}\n" --color=debug
  [red|a/1/in_a_1]
  [red|a/in_a]
  [red|b/1/in_b_1]
  [red|b/2/in_b_2]
...
Make sure ui.formatted=False works
  $ sl status --color=auto --config ui.formatted=False
  ? a/1/in_a_1
  ? a/in_a
  ? b/1/in_b_1
  ? b/2/in_b_2
...
templates should be.
  ...
  $ sl log --color=debug -T '{label("my.label",author)}\n{label("skipped.label","")}'
  [my.label|test]
...
test unknown color
  ...
  $ EDENSCM_LOG=termstyle=warn sl --config color.status.modified=periwinkle status
...
sl resolve with one unresolved, one resolved:
...
color coding of error message without curses
  ...
  $ PYTHONPATH=`pwd`:$PYTHONPATH sl unknowncommand > /dev/null
  unknown command 'unknowncommand'
  (use 'sl help' to get help)
  [255]
