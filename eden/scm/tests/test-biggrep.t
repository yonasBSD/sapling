...
Test filtered output with file pattern:
  $ sl grep --color=off foobar grepdir/subdir1 | sort
  grepdir/subdir1/subfile1:foobar_subdir_bg

Test dash escaping produces correct match:
  $ sl grep --color=off -- -g | sort
...
Test from subdirectory with relative paths:
  ...
  $ sl grep --color=off -n foobar ../subdir2 | sort
...
Create a second commit with changes:
  ...
  $ sl rm grepdir/grepfile2
  $ sl add grepdir/newfile
  $ sl commit -m "Second commit: add newfile, modify grepfile1, remove grepfile2"

Set corpus to the first commit - biggrep returns stale data:
  $ BIGGREP_CORPUS_REV='.^' sl grep --color=off foobar | sort
...
Test with line numbers:
  $ BIGGREP_CORPUS_REV='.^' sl grep --color=off -n foobar | sort
...
Define BIGGREP_FILES for second commit state:
  ...
  $ BIGGREP_FILES="$BGFILES_V2" BIGGREP_CORPUS_REV='max(all())' sl grep --color=off foobar | sort
...
Make some uncommitted modifications:
  ...
  $ sl add grepdir/uncommitted_file
  $ sl rm grepdir/grepfile3

Corpus points to current commit (second commit), but we have uncommitted changes:
  $ BIGGREP_FILES="$BGFILES_V2" sl grep --color=off foobar | sort
...
Search the first commit when corpus points to current commit:
  $ BIGGREP_FILES="$BGFILES_V2" BIGGREP_CORPUS_REV='.' sl grep --color=off -r "$COMMIT1" foobar | sort
  grepdir/grepfile1:foobarbaz
  grepdir/grepfile2:foobarboo
  grepdir/subdir1/subfile1:foobar_subdir_bg
  grepdir/subdir2/subfile2:foobar_dirsub_bg
