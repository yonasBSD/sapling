...
First client: make commit and push to scratch branch
  ...
  $ sl push -q -r . --to scratch/newscratch --create
  $ cd ..

Second client: pull scratch commit and update to it
  $ cd shallow2
  $ sl pull -q -B scratch/newscratch
  $ sl up 2d9cfa751213
...
First client: make commits with file modification and file deletion
  ...
  $ sl ci -Aqm 'scratch commit with many files'
  $ sl rm dir/toremove
  $ sl ci -Aqm 'scratch commit with deletion'
  $ sl push -q -r . --to scratch/newscratch --force
  $ cd ..

Second client: pull new scratch commits and update to all of them
  $ cd shallow2
  $ sl pull -q --config remotefilelog.excludepattern=somefile -B scratch/newscratch
  $ sl up 70ec84a579b5
...
First client: make a file whose name is a glob
  $ cd shallow1
  $ echo >> foo[bar]
  $ sl commit -Aqm "Add foo[bar]"
  $ echo >> foo[bar]
  $ sl commit -Aqm "Edit foo[bar]"
  $ sl push -q -r . --to scratch/regex --create
  $ cd ..

Second client: pull regex file an make sure it is readable
(only pull the first commit, to force a rebundle)
  $ cd shallow2
  $ sl pull -q -r 3109e6519e25
  $ sl log -r 3109e6519e25 --stat
  commit:      3109e6519e25
  user:        test
