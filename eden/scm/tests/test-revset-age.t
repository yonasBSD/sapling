...
# Prepare commits
  ...
  ...     cmd = f"sl commit -d '{now-delta} 0' -m 'Changeset {delta} seconds ago' -A file1"
  ...     sheval(cmd) or None

# Check age ranges

    from sapling.extensions import wrappedfunction
    with wrappedfunction(time, "time", lambda orig: now + 1):
      ...
      3 Changeset 3800 seconds ago
      $ sl log -T '{rev} {desc}\n' -r 'age("invalid")'
      sl: parse error: invalid age range
      [255]
      $ sl log -T '{rev} {desc}\n' -r 'age("1h")'
      sl: parse error: invalid age range
      [255]
      $ sl log -T '{rev} {desc}\n' -r 'age("<3m2h")'
      sl: parse error: invalid age in age range: 3m2h
      [255]
      $ sl log -T '{rev} {desc}\n' -r 'age(">3h2h")'
      sl: parse error: invalid age in age range: 3h2h
      [255]
      $ sl log -T '{rev} {desc}\n' -r 'age("1h-5h-10d")'
      sl: parse error: invalid age in age range: 5h-10d
      [255]
      $ sl log -T '{rev} {desc}\n' -r 'ancestorsaged(., "<1d")'
      2 Changeset 86369 seconds ago
      3 Changeset 3800 seconds ago
