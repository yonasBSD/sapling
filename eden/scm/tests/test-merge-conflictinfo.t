...
2) Can't run dumpjson outside a conflict
  $ sl resolve --tool internal:dumpjson
...
3) Make a simple conflict
  ...
  $ sl commit -m "second version, b"
  $ sl bookmark b
  $ sl log -G -T '({node}) {desc}\nbookmark: {bookmarks}\nfiles: {files}\n\n'
  ...
  $ sl merge a
  merging F1
  warning: 1 conflicts while merging F1! (edit, then use 'sl resolve --mark')
  merging F2
  warning: 1 conflicts while merging F2! (edit, then use 'sl resolve --mark')
...
6) Only requested paths get dumped
  $ sl resolve --tool internal:dumpjson F2 | pp
  [
    {
      "command": "merge",
      "command_details": {
...
7) Ensure the paths point to the right contents:
  ...
  >  sl resolve --tool internal:dumpjson --all | sl debugsh -c "$script"
  > }
...
Test case 4: Source changed, dest moved (w/o copytracing)
  ...
  $ sl rebase -d 'desc(dest)' -s 'desc(source)' --config copytrace.sourcecommitlimit=0
  rebasing ec87889f5f90 "source"
  other [source (being rebased)] changed file which local [dest (rebasing onto)] is missing
