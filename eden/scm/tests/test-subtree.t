...
test subtree copy paths validation
  ...
  [255]
  $ sl subtree copy -r $A --from-path foo
  abort: must provide same number of --from-path ['foo'] and --to-path []
  [255]
  $ sl subtree copy -r $A --from-path bar
  abort: must provide same number of --from-path ['bar'] and --to-path []
  [255]
  $ sl subtree copy -r $A --from-path foo --to-path bar --from-path foo --to-path ""
  abort: overlapping --to-path entries
  [255]
  $ sl subtree copy -r $A --from-path nonexist --to-path bar
  abort: path 'nonexist' does not exist in commit d908813f0f7c
  [255]

test subtree copy source commit validation
  $ sl subtree cp -r $A --from-path foo --to-path bar --config subtree.allow-any-source-commit=False
  ...
  (hint: see 'sl help subtree' for the impacts on subtree merge and log)
  Continue with subtree copy (y/n)?  n
  abort: subtree copy from a non-public commit is not allowed
  [255]
  $ sl subtree cp -r $A --from-path foo --to-path bar --config subtree.allow-any-source-commit=False --config subtree.education-page=https://abc.com/subtree
  subtree copy from a non-public commit is not recommended. However, you can
  still proceed and use subtree copy and merge for common cases.
...
test subtree copy
  ...
  > EOS
  $ sl go $B -q
  $ sl subtree cp -r $A --from-path foo --to-path bar -m "subtree copy foo -> bar"
  copying foo to bar
...
test subtree copy metadata sorted by to-path
  ...
  > EOS
  $ sl go $B -q
  $ sl subtree cp -r $A --from-path foo --to-path bar --from-path foo2 --to-path baa -m "subtree copy foo -> bar and foo2 -> baa"
  copying foo to bar
  copying foo2 to baa
  $ sl log -G -T '{node|short} {desc|firstline}\n'
...
test subtree copy without skipping source commit check: new commit does not have subtree metadata
  ...
  $ sl subtree cp -r $A --from-path foo --to-path bar -m "subtree copy foo -> bar"<<EOF
  > y
  > EOF
  subtree copy from a non-public commit is not recommended. However, you can
  still proceed and use subtree copy and merge for common cases.
...
abort when subtree copy too many files
  ...
  >     # drawdag.defaultfiles=false
  > EOS  
  $ sl subtree cp -r $A --from-path foo --to-path bar --config subtree.max-file-count=1
  abort: path 'foo' includes too many files: 2 (max: 1)
  [255]
  $ sl subtree cp -r $A --from-path foo --to-path bar --config subtree.max-file-count=1 --config ui.supportcontact="Sapling Team"
  abort: path 'foo' includes too many files: 2 (max: 1)
...
test max file count for multiple from paths, check each from path separately
  ...
  >     # drawdag.defaultfiles=false
  > EOS
  $ sl subtree cp -r $A --from-path bar --to-path bar2 --from-path foo --to-path foo2 --config subtree.max-file-count=1
  abort: path 'foo' includes too many files: 2 (max: 1)
  [255]
  $ sl subtree cp -r $A --from-path bar --to-path bar2 --from-path foo --to-path foo2 --config subtree.max-file-count=2
  copying bar to bar2
  copying foo to foo2
...
abort when the working copy is dirty
  ...
  $ sl st
  M foo/x
  $ sl subtree cp -r $A --from-path foo --to-path bar
...
test subtree copy with symlinks
  ...
  $ echo "bbb" > foo/a
  $ sl ci -m 'second'
  $ sl subtree cp -r "desc(first)" --from-path foo --to-path foo2
  copying foo to foo2
  $ readlink foo2/b
  a
  $ cat foo2/b
...
test subtree copy with execs
  ...
  foo2/a: mode=755
  foo2/b: mode=644
  $ sl dbsh -c 'for x in ["foo/a", "foo/b", "foo2/a", "foo2/b"]: print([x, repo["."].manifest().flags(x)])'
  ['foo/a', 'x']
  ['foo/b', '']
  ['foo2/a', 'x']
  ['foo2/b', '']
...
test subtree copy to tracked directory
  ...
  [255]
  $ sl subtree cp -r $A --from-path foo --to-path bar --force
  removing bar/x
  copying foo to bar
  $ cat bar/x
  aaa
...
test subtree graft
  ...
  > EOS
  $ sl go $C -q
  $ sl subtree copy -r $B --from-path foo --to-path bar -m 'subtree copy foo -> bar'
  copying foo to bar

  $ sl subtree graft -r $C
  abort: must provide --from-path and --to-path for same-repo grafts
  [255]
  $ sl subtree graft -r $C --from-path foo
  abort: must provide --from-path and --to-path for same-repo grafts
  [255]
  $ sl subtree graft -r $C --to-path bar
  abort: must provide --from-path and --to-path for same-repo grafts
  [255]

  $ sl subtree graft -r $C --from-path foo --to-path bar
...
test 'subtree graft -m'
  ...
  $ sl subtree graft -r $C --from-path foo --to-path bar -m "new C"
  grafting 78072751cf70 "C"
  merging bar/x and foo/x to bar/x
  $ sl show
  commit:      6592d0497ef7
...
Test 'subtree graft -m' with --no-log
  ...
  $ sl subtree graft --no-log -r $C --from-path foo --to-path bar -m "new C\
  > \
  > Test Plan:\
  > \
  > test 123"
...
test subtree merge
  ...
  $ sl subtree copy -r $B --from-path foo --to-path bar -m 'subtree copy foo -> bar'
  copying foo to bar
  $ sl subtree merge -r $D --from-path foo --to-path bar
  ...
  +4
  $ sl continue
  abort: cannot continue with 'sl commit' in non-interactive mode
  (use 'sl commit' to commit or 'sl status' for more info)
  [255]
...
test subtree merge with normal copy
  ...
  > EOS
  $ sl go $C -q
  $ sl subtree merge -r $C --from-path foo --to-path bar
  searching for merge base ...
  ...
  $ sl st
  M bar/x
  $ sl debugmergestate | grep -B 1 -A 2 "subtree merges"
...
test subtree merge with no common base
  ...
  merging bar/x and foo/x to bar/x
  warning: 1 conflicts while merging bar/x! (edit, then use 'sl resolve --mark')
