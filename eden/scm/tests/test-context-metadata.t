...
Tests about metadataonlyctx
  ...
  $ sl init
  $ echo A > A
  $ sl commit -A A -m 'Add A'
  $ echo B > B
  $ sl commit -A B -m 'Add B'
  $ sl rm A
  $ echo C > C
  $ echo B2 > B
  $ sl add C -q
  ...
  >         new.commit()
  > EOF
  $ sl --config extensions.metaedit=$TESTTMP/metaedit.py metaedit 'text=Changed'
  $ sl log -r tip
  commit:      ad83e9e00ec9
  user:        test
  ...
  $ NODE_ADD_A=$(sl log -r 'desc("Add A")' -T '{node}')
  $ sl --config extensions.metaedit=$TESTTMP/metaedit.py metaedit "parents=$NODE_ADD_A" 2>&1 | egrep '^RuntimeError'
  ...
  $ sl --config extensions.metaedit=$TESTTMP/metaedit.py metaedit 'user=foo <foo@example.com>'
  $ sl log -r tip
  commit:      1f86eaeca92b
