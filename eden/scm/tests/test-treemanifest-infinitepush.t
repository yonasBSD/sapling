...
Push a non-tree scratch branch from one client
  ...
  $ mkdir bar
  $ echo >> bar/car
  $ sl commit -qAm 'add bar/car'
  $ echo >> bar/car
  $ sl commit -qm 'edit bar/car'
  $ sl push -q --to scratch/nontree --create
  $ clearcache
  $ cd ..
...
Push a tree-only scratch branch from another client
  ...
  $ sl commit -qAm 'add subdir/a'
  $ echo "my other change" >> subdir/a
  $ sl commit -qAm 'edit subdir/a'
  $ sl push -q --to scratch/foo --create
  $ cd ..
...
Pull just part of a treeonly scratch branch (this causes rebundling on the server)
  ...
  $ sl pull -r 02c12aef64ff
  pulling from ssh://user@dummy/master
  searching for changes
  $ sl log -r 02c12aef64ff  --stat
  commit:      02c12aef64ff
  user:        test
  date:        Thu Jan 01 00:00:00 1970 +0000
...
Verify sl cloud backup in a treeonly client will convert old flat manifests into
trees
...
# Merge the root into master and push the merge as a backup
  $ sl up -q f027ebc7ba78
  2 files fetched over 1 fetches - (2 misses, 0.00% hit ratio) over * (glob) (?)
  $ sl merge d32fd17cb041
