#require git no-eden

  $ export HGIDENTITY=sl
  $ eagerepo
  $ . $TESTDIR/git.sh

Make a commit with a non-GMT timezone:

  $ sl init --git gitrepo1
  $ cd gitrepo1
  $ sl commit -d '2022-11-23 17:47:30 -0800' -m A --config ui.allowemptycommit=1
  $ sl bookmark -q A

Timezone parsed by hg:

  $ sl log -r . -T '{date|isodatesec}\n'
  2022-11-23 17:47:30 -0800

Template keywords. Note the committerdate is a constant 0 in tests:

  $ sl log -r . -T '{authordate|isodatesec}\n{committerdate|isodatesec}\n'
  2022-11-23 17:47:30 -0800
  1970-01-01 00:00:00 +0000

Timezone parsed by Git:

  $ git --git-dir=.sl/store/git log --format=%ai refs/heads/A
  2022-11-23 17:47:30 -0800
