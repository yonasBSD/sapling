...
# Merge commit will not be changed:
  ...
  $ sl goto -q '.^'
  $ echo 2 > m2
  $ sl commit -q -A m2 -m m2
  $ sl merge -q m1
  $ sl commit -m merge
  $ sl bookmark -d m1
  $ sl log -G -T '{rev} {desc} {phase}\n'
  @    25 merge draft
  ├─╮
  │ o  24 m2 draft
  │ │
...
# Test config option absorb.maxstacksize:
  ...
  $ sl --config 'absorb.maxstacksize=1' absorb -n
...
# Test obsolete markers creation:
  ...
  $ sl --config 'absorb.maxstacksize=3' sf -a
  ...
  b171794 commit a 2
  2 of 2 chunks applied
  $ sl log -T '{rev}:{node|short} {desc} {get(extras, "absorb_source")}\n'
...
# Test config option absorb.amendflags and running as a sub command of amend:
  ...
  Options:
    ...
    --correlated incorporate corrections into stack. see 'sl help absorb' for
                 details
...
# Test binary file
  ...
  $ sl commit -A c -m 'c is a binary file'
  $ echo c >> c
  ...
  $ echo END >> b
  $ sl rm a
  $ echo y | sl amend --correlated --config 'ui.interactive=1'
  showing changes for b
          @@ -1,0 +1,1 @@
          +INS
          @@ -2,0 +3,1 @@
