...
for simplicity):
  $ sl eval "context.arbitraryfilectx('A', repo).cmp(repo[None]['real_A'])"
  True (no-eol)
  $ sl eval "not filecmp.cmp('A', 'real_A')"
  True (no-eol)

These files are identical and should return False (same):
  $ sl eval "context.arbitraryfilectx('A', repo).cmp(repo[None]['A'])"
  False (no-eol)
  $ sl eval "context.arbitraryfilectx('A', repo).cmp(repo[None]['B'])"
  False (no-eol)
  $ sl eval "not filecmp.cmp('A', 'B')"
  False (no-eol)
...
the same in the eyes of ``filectx.cmp``, which looks at data only.
  $ sl eval "context.arbitraryfilectx('real_A', repo).cmp(repo[None]['sym_A'])"
  False (no-eol)
...
#if symlink
  $ sl eval "not filecmp.cmp('real_A', 'sym_A')"
  True (no-eol)
#else
  $ sl eval "not filecmp.cmp('real_A', 'sym_A')"
  False (no-eol)
