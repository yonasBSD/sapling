...
test rebase subtree copy commit fails if the to-path is updated on the dest side
  ...
  (use 'sl subtree copy' to re-create the directory branch. Please see https://abc.com/sapling-subtree-copy-conflict for more help)
  [255]
...
test rebase subtree copy commit does not merge source changes
  ...
  $ ls foo2
  x
  $ sl subtree copy -r $B --from-path foo --to-path foo2 -m "subtree copy foo to foo2" --force
  removing foo2/x
  copying foo to foo2
  $ ls foo2
  x
