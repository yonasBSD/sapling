#require no-eden
  ...
  $ showgraphother() {
  >    local OTHER="$1"
  >    local CURRENT_REV=$(sl id)
  >    sl cloud switch -w "$OTHER" --force -q
  >    sl up $Z -q
  >    sl log -G -T "{desc}: {phase} {bookmarks} {remotenames}" -r "all()"
  >    sl cloud switch --force -q
  >    sl up "$CURRENT_REV" -q
  > }

sl cloud move uses the smartlog data from the cloud service.  We must generate this
manually.
  $ gensmartlogdata() {
...
Move by bookmark leaves commits alone if there are other bookmarks. The moved bookmark should be just added to the destination workspace.
  $ gensmartlogdata
  $ sl cloud move -d other -B d-bookmark
  commitcloud: moving requested commits and bookmarks from 'user/test/default' to 'user/test/other' workspace for the 'server' repo
  moving bookmarks:
...
But moving all of the bookmarks pointing to a head removes the head from the source workspace.
  $ gensmartlogdata
  $ sl cloud move -d other -B "re:d-bookmark.*"
  commitcloud: moving requested commits and bookmarks from 'user/test/default' to 'user/test/other' workspace for the 'server' repo
  moving heads:
      fa9d7a2f38d1  D
  moving bookmarks:
...
Moving a bookmark in the stack doesn't hide the commit in the source workspace.
  $ gensmartlogdata
  $ sl cloud move -d other -B b-bookmark
  commitcloud: moving requested commits and bookmarks from 'user/test/default' to 'user/test/other' workspace for the 'server' repo
  moving bookmarks:
...
Moving a bookmark on a public commit just moves it.
  $ gensmartlogdata
  $ sl cloud move -d other -B x-bookmark
  commitcloud: moving requested commits and bookmarks from 'user/test/default' to 'user/test/other' workspace for the 'server' repo
  moving bookmarks:
...
Moving a remote bookmark works.
  $ gensmartlogdata
  $ sl cloud move -d other --remotebookmark remote/other
  commitcloud: moving requested commits and bookmarks from 'user/test/default' to 'user/test/other' workspace for the 'server' repo
...
Merge commits can be moved
  ...
  $ gensmartlogdata
  $ sl cloud move -d othermerge $H --create
...
Try to move the same stack twice
  $ gensmartlogdata
  $ sl cloud move -d movetwice --create $B
  ...
  $ gensmartlogdata
  $ sl cloud move -s default -d movetwice $B
  commitcloud: moving requested commits and bookmarks from 'user/test/default' to 'user/test/movetwice' workspace for the 'server' repo
  moving heads:
      080f94b3ed7f  F
  commitcloud: synchronizing 'server' with 'user/test/default'
...
Try move with specified raw source and raw destination
  $ gensmartlogdata
  $ sl cloud move --raw-source user/test/default --raw-destination user/test/rawtest --create $G
...
Test copying commits and bookmarks between workspaces
  ...
  finished in * (glob)
  $ gensmartlogdata
  $ sl cloud copy -r $B -d copytest --create
