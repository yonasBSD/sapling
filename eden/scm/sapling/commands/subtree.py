...
def subtree_inspect(ui, repo, *args, **opts):
    ...
    xmerges = get_subtree_xmerges(repo, ctx.node())
    ...
    if xmerges:
        result["xmerges"] = [c.to_full_dict() for c in xmerges]
...
def _do_import(ui, repo, *args, **opts):
    ...
    summaryfooter = subtreeutil.gen_import_commit_msg(
        url, from_commit, from_paths, to_paths
    )
