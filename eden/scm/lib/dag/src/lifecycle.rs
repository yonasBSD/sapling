...
/// - With `tracing-backtrace` (`SL_BTLOG=dag::lifecycle::create=debug`),
///   prints backtrace on new, clone, drop.
///
/// Example uses:
...
impl LifecycleId {
    ...
    pub(crate) fn new_with_type_name(type_name: &'static str) -> Self {
        ...
        tracing::debug!(target: "dag::lifecycle::create", type_name = type_name, id = id);
        Self { id, type_name }
    }
}
...
impl Drop for LifecycleId {
    fn drop(&mut self) {
        let type_name = self.type_name;
        let id = self.id;
        tracing::debug!(target: "dag::lifecycle::drop", type_name = type_name, id = id);
    }
}
